...
def banner():
    ...
    print(BRIGHT + GREEN)
    print(r"  ___ ___  _   __  __  ")
    print(r" / __| _ \/_\ |  \/  | ")
    print(r" \__ \  _/ _ \| |\/| | ")
    print(r" |___/_|/_/ \_\_|  |_| ")
    print()
    print(r"     Chinese localization/Spammer: github.com/HugoXOX3      ")
    print(RESET_ALL)
...
def main():
    ...
    print("[3] 退出.")
    print()
    number = input(f"{BRIGHT}{BLUE}输入项目编号: {RESET_ALL}")
    if number == "1":
        spam_handler()
    elif number == "2":
        update()
...
def spam_handler():
    ...
    try:
        ...
        threads = "1"
    banner()
    print(f"电话: {BRIGHT}{BLUE}{phone}{RESET_ALL}")
    print("垃圾邮件发送者推出", end="\n")
    print(f"{BRIGHT}{RED}[*] Ctrl+Z 停止{RESET_ALL}")
...
def check_version():
    ...
    if float(version) < float(get("https://raw.githubusercontent.com/cludeex/spammer/master/version.txt").text):
        print(f"\n{BRIGHT}{RED}[*] 版本已过时，需要更新!{RESET_ALL}")
        sleep(1)
        main()
    return
